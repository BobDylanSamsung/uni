...
int main(int argc, char ** argv) {
    ...
    //send order
    order = "SELL 5 ROUTER 999999 999999;";
    strncpy(buf, order, strlen(order));
    write(fd_write, buf, strlen(order));
    kill(exchange_pid, SIGUSR1);
    // wait for exchange confirmation (ACCEPTED message)
